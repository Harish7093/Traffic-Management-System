...
with col2:
    video_path=os.path.join(homepage_img_dir,"raw_clip.mp4")
    video_file = open(video_path, 'rb')
    video_bytes = video_file.read()
    
    st.video(video_bytes)
    st.success("Real Time Pothole Detection using YOLO algorithm ")
st.success("Team Members")

st.markdown("""
            - K. Harish Kumar  - 99220040566 (CSE)
            - D. Khasim Aman   - 99220040496 (CSE)
            - M. Sai           - 99220040622 (CSE)
            - V. Pavan Kumar   - 99220041592 (CSE)
            - M. Mugesh Kannan - 9823005008  (ECE)
